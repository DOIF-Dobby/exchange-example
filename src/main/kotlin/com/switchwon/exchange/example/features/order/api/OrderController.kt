...
class OrderController(
     ...
     */
    @PostMapping("/orders")
    fun order(@CurrentMemberId memberId: MemberId, @RequestBody @Valid request: OrderRequest): UnitApiResponse {
        orderService.order(
            memberId = memberId,
            request = request
        )
     ...
     */
    @GetMapping("/orders")
    fun getOrders(@CurrentMemberId memberId: MemberId): ApiResponse<List<OrderResponse>> {
        val orders = orderFindService.findOrderResponsesByMemberId(memberId)
    ...
    fun getQuote(@ModelAttribute request: OrderQuoteRequest): ApiResponse<OrderQuoteResponse> {
        val krwAmount = orderQuoteService.getQuote(request)
        return ApiResponse.ok(OrderQuoteResponse(krwAmount = krwAmount))
    }
}
